package io.github.candycalc.casting.mishap
...
class MishapBadArmorItem(val item: ItemStack?, val wanted: MutableText, val slot: ArmorItem.Type) : Mishap() {
    override fun accentColor(ctx: CastingEnvironment, errorCtx: Context): FrozenPigment =
        dyeColor(DyeColor.BROWN)

    override fun errorMessage(ctx: CastingEnvironment, errorCtx: Context)  = if (item?.isEmpty == false)
        error("bad_item.armor", wanted, slot.name, item.name)
    else
        error("no_item.armor", wanted, slot.name)
